...
def main():
    ...
    add_umap_arrows(ax)
    plt.tight_layout()
    pub_style.save_pub_fig(plt.gcf(), OUT_DIR / "UMAP_Unannotated_Clusters.tiff", dpi=300, format='tiff')
    plt.close()
    ...
    if 'infection_state' in adata.obs:
        ...
        add_umap_arrows(ax)
        plt.tight_layout()
        pub_style.save_pub_fig(plt.gcf(), OUT_DIR / "UMAP_Annotated_States.tiff", dpi=300, format='tiff')
        plt.close()
        ...
        if genes_to_plot:
            ...
            plt.tight_layout()
            pub_style.save_pub_fig(plt.gcf(), OUT_DIR / "Validation_Feature_Plots.tiff", dpi=300, format='tiff')
            plt.close()
        ...
        dp.savefig(OUT_DIR / "DotPlot_Example.tiff", dpi=300, format='tiff')
        pass  # Execution logging removed for final release
