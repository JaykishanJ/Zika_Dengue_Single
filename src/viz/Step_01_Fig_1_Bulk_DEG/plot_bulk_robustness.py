...
def plot_deg_counts():
    datasets = {
        "ZIKV (Macrophage)": BULK_DIR / "bulk_GSE118305_ZIKV_macrophage_DE.csv",
        "ZIKV (Huh7)": BULK_DIR / "GSE78711/Results/Tables/ZIKV_positive_vs_Mock_All_Genes.csv",
        "DENV (Blood)": BULK_DIR / "bulk_GSE279208_DENV_blood_DE.csv"
    }
    
    counts = []
    for name, path in datasets.items():
        if path.exists():
            df = pd.read_csv(path).dropna(subset=['padj', 'lfc'])
            up = len(df[(df['padj'] < 0.05) & (df['lfc'] > 1)])
            down = len(df[(df['padj'] < 0.05) & (df['lfc'] < -1)])
...
def plot_logfc_correlation():
    ...
    df_zikv = pd.read_csv(zikv_path).dropna(subset=['Gene', 'lfc'])
    df_denv = pd.read_csv(denv_path).dropna(subset=['Gene', 'lfc'])
    ...
    df_merged = pd.merge(df_zikv[['Gene', 'lfc']], df_denv[['Symbol', 'lfc']], left_on='Gene', right_on='Symbol', suffixes=('_ZIKV', '_DENV'))
