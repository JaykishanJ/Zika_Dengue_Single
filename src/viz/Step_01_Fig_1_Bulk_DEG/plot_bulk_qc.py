...
def plot_ma(df, title, out_path):
    df = df.dropna(subset=['baseMean', 'lfc', 'padj']).copy()
    ...
    # Plot non-sig first
    sns.scatterplot(data=df[df['Sig']=='Not Significant'], x='baseMean', y='lfc', color=palette['Not Significant'], s=10, alpha=0.3, edgecolor=None, ax=ax)
    sns.scatterplot(data=df[df['Sig']=='Up'], x='baseMean', y='lfc', color=palette['Up'], s=15, alpha=0.8, edgecolor='black', linewidth=0.2, ax=ax)
    sns.scatterplot(data=df[df['Sig']=='Down'], x='baseMean', y='lfc', color=palette['Down'], s=15, alpha=0.8, edgecolor='black', linewidth=0.2, ax=ax)
