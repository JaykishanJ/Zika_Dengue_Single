...
def plot_volcano(df, title, out_path, is_dengue=False):
    df = df.dropna(subset=['padj', 'lfc', 'Gene']).copy()
    ...
    df.loc[(df['padj'] < 0.05) & (df['lfc'] > 1), 'Sig'] = 'Up'
    df.loc[(df['padj'] < 0.05) & (df['lfc'] < -1), 'Sig'] = 'Down'
    ...
    sns.scatterplot(data=df[df['Sig']=='Not Significant'], x='lfc', y='nlog10', color=palette['Not Significant'], s=15, alpha=0.5, edgecolor=None, ax=ax)
    sns.scatterplot(data=df[df['Sig']=='Up'], x='lfc', y='nlog10', color=palette['Up'], s=25, alpha=0.8, edgecolor='black', linewidth=0.2, ax=ax)
    sns.scatterplot(data=df[df['Sig']=='Down'], x='lfc', y='nlog10', color=palette['Down'], s=25, alpha=0.8, edgecolor='black', linewidth=0.2, ax=ax)
    ...
    for _, row in pd.concat([top_up, top_down]).iterrows():
        texts.append(ax.text(row['lfc'], row['nlog10'], row['Symbol'], fontsize=9, fontweight='bold'))
...
def main():
    ...
    zikv_path = BULK_DIR / "bulk_GSE118305_ZIKV_macrophage_DE.csv"
    if zikv_path.exists():
    ...
    pass  # Execution logging removed for final release
    denv_path = BULK_DIR / "bulk_GSE279208_DENV_blood_DE.csv"
    if denv_path.exists():
