...
def main():
    ...
    deg_df = pd.read_csv(config.PROCESSED_DATA_DIR / "Step06_crossmodal_validation/crossmodal_validated_progression_signature.csv")
    core_genes = deg_df[deg_df['tier'] == 1]['symbol'].dropna().unique().tolist()
    pass  # Execution logging removed for final release
    ...
    pass  # Execution logging removed for final release
    expr_df = pd.read_csv(BASE_DIR / "raw_data/bulk/GSE279208/GSE279208_VST_Normalized_Matrix.csv", index_col=0)
    pheno_df = pd.read_csv(config.PROCESSED_DATA_DIR / "Step05b_bulk_differential_expression/GSE279208/Results/Annotation/GSE279208_Final_Phenotype.csv")
